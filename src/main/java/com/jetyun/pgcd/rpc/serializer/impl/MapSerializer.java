...
import com.jetyun.pgcd.rpc.serializer.JSONSerializer;
...
/**
 * 类说明: 将 json转换为Map对象及将Map对象转换为json串的方法<br>
 * 如果需要将json 转换为 Map ,则传入的json 的格式为
 * {javaClass:'java.util.HashMap',map:{name:'susan'}} 如果将Map转换为json ，则最终的结构格式为
 * {name:'susan'} 创建时间: 2013-1-9 下午04:05:51<br>
 * 
 * @author 李祥辉<br>
...
public class MapSerializer extends AbstractSerializer {
	...
	public Object marshall(Object p, Object o) throws MarshallException {
		...
		try {
			...
			throw (MarshallException) new MarshallException("map key " + key
	...
	public ObjectMatch tryUnmarshall(Class clazz, Object o)
		...
		try {
			while (i.hasNext()) {
				key = (String) i.next();
				m.setMismatch(ser.tryUnmarshall(null, jsonmap.get(key)).max(m)
						.getMismatch());
			}
