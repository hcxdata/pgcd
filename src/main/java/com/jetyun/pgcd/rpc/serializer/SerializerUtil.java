...
import com.jetyun.pgcd.util.DateTimeUtils;
import com.jetyun.pgcd.util.DateUtil;

/**
 * @ File_name SerializerUtil.java @ Author : LiXiangHui (sino.soft@163.com) @
 * CreateDate: 2009-9-18 上午10:54:43 @ Description : @ Reviewed : @ UpateLog:
 * Name Date Reason/Contents --------------------------------------- *** ****
 * ****
 */
public class SerializerUtil {
	static String[] baseType = new String[] { "char", "short", "int", "long",
			"float", "boolean", "Character", "Short", "Integer", "Long",
	 ...
	 *            日期转换时的格式
	 */
	public static void object2Json(Object o, JSONObject json, String[] args,
		...
		while (iter.hasNext()) {
			...
			if (!ArrayUtils.contains(args, parameter)) {
				if (map.getReadMethod(parameter) != null
						&& map.getReadMethod(parameter).getReturnType()
								.isAssignableFrom(Date.class))// 日期类型特殊处理
				{
					json.put(parameter, DateUtil.formatDate(
							(Date) map.get(parameter), format));
					continue;
				}
				if (ArrayUtils.contains(baseType, map.getType(parameter)
						.getSimpleName())) // 只处理基本类型
		...
		// Arrays.asList(args).indexOf(mehtod2Parameter(method.getName()))==-1)
		// {
		// if(method.getReturnType().getSimpleName().equals("Date")) //日期类型特殊处理
		// {
		// json.put(mehtod2Parameter(method.getName()),
		// DateUtil.formatDate((Date)method.invoke(o, new Object[0]),format));
		// continue;
		// }
		// if(Arrays.asList(baseType).indexOf(method.getReturnType().getSimpleName())!=-1)
		// //只处理基本类型
		// json.put(mehtod2Parameter(method.getName()), method.invoke(o, new
		// Object[0]));
		// }
		// }
	}
	 ...
	 *            装载的json对象
	 */
	public static void object2Json(Object o, JSONObject json)
	 ...
	 *            日期转换时的格式
	 */
	public static void object2Json(Object o, JSONObject json, String format)
			throws IllegalArgumentException, IllegalAccessException,
			InvocationTargetException, JSONException {
	 ...
	 *            日期转换时的格式
	 */
	public static <T> void json2Object(JSONObject json, T object,
			String[] args, String format) throws JSONException,
		...
		while (iter.hasNext()) {
			...
			if (setMethod != null && !ArrayUtils.contains(args, name)) {
				if (json.has(name)
						&& StringUtils.isNotBlank(json.getString(name))) {
					...
					if (type.isAssignableFrom(Date.class)
						...
						setMethod.invoke(object, DateTimeUtils
								.convertStringToDateTime(format,
										json.getString(name)));
	 ...
	 *            日期转换时的格式
	 */
	public static <T> void json2Object(JSONObject json, T object, String format)
			throws IllegalArgumentException, JSONException,
			IllegalAccessException, InvocationTargetException, ParseException {
	 ...
	 *            日期转换时的格式
	 */
	public static <T> void json2Object(JSONObject json, T object, String[] args)
			throws IllegalArgumentException, JSONException,
			IllegalAccessException, InvocationTargetException, ParseException {
