...
/**
 * 类说明: 将 json转换为Map对象及将Set对象转换为json串的方法<br>
 * 如果需要将json 转换为 Set ,则传入的json 的格式为
 * {javaClass:'java.util.HashSet',set:{name:'susan'}} 如果将Set转换为json ，则最终的结构格式为
 * {set:{susan:'susan'}} 创建时间: 2013-1-9 下午04:05:51<br>
 * 
 * @author 李祥辉<br>
...
public class SetSerializer extends AbstractSerializer {
	...
	public ObjectMatch tryUnmarshall(Class clazz, Object o)
		...
		try {
			while (i.hasNext()) {
				key = (String) i.next();
				m.setMismatch(ser.tryUnmarshall(null, jsonset.get(key)).max(m)
						.getMismatch());
			}
