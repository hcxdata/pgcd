package com.jetyun.pgcd.rpc.serializer.impl;
...
import com.jetyun.pgcd.rpc.serializer.AbstractSerializer;
import com.jetyun.pgcd.rpc.serializer.MarshallException;
import com.jetyun.pgcd.rpc.serializer.ObjectMatch;
import com.jetyun.pgcd.rpc.serializer.UnmarshallException;
import com.jetyun.pgcd.util.DateUtil;

/**
 * 类说明: 将 json转换为Date对象及将Date对象转换为json串的方法<br>
 * 如果需要将json 转换为 Date ,则传入的json 的格式为
 * {javaClass:'java.util.Date',time:'34893492384'} 或
 * {javaClass:'java.util.Date',date:'20120131'} 如果希望自定义格式化方式，则json中提供format 属性即可
 * Ex:{javaClass:'java.util.Date',date:'20120131',format:'yyyyMMdd'}
 * 如果将Date转换为json ，则最终的结构格式为
 * {"date":"20111231","time":{"date":31,"day":6,"hours"
 * :8,"minutes":0,"month":11,
 * "seconds":0,"time":1325289600000,"timezoneOffset":-480,"year":111}} 创建时间:
 * 2013-1-9 下午04:05:51<br>
 * 
 * @author 李祥辉<br>
...
public class DateSerializer extends AbstractSerializer {
	...
	public Object marshall(Object p, Object o) throws MarshallException {
		...
		try {
			...
			throw (MarshallException) new MarshallException(e.getMessage())
					.initCause(e);
		}
		return obj;
	}
	...
	public Object unmarshall(Class clazz, Object o) throws UnmarshallException {
		...
		if (!jso.has("time") && !jso.has("date"))
			throw new UnmarshallException(
					"Could not get the time or date in date serialiser");
		...
		if (jso.has("date") && Date.class.equals(clazz)) {
			...
			try {
				...
				throw new UnmarshallException("cant get date from json : "
						+ jso.toString(), e);
			} catch (Exception e) {
				throw new UnmarshallException("cant pase string to Date : "
						+ timestr, e);
			}
			return returnValue;
		}
		try {
			...
			throw new UnmarshallException("cant case time to long :"
					+ jso.toString(), e);
		}
