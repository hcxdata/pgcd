...
import com.jetyun.pgcd.util.JsonRpcUtil;
import com.jetyun.pgcd.util.ResponseUtils;
...
public class BaseActionController extends AbstractController {
	...
	protected ModelAndView handleRequestInternal(HttpServletRequest request,
		...
		// 进行调用
		try {
			json_req = JsonRpcUtil.changeParameterMapToJson(request
					.getParameterMap());
		...
		if (request.getParameter("result") != null
				&& "direct".equals(request.getParameter("result")
						.replaceAll("\"", "").replace("'", ""))
				&& json_res.getErrorCode() == JSONRPCResult.CODE_SUCCESS) {
